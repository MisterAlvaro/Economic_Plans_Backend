...
exports.validateExcelUpload = validateExcelUpload;
...
function validateExcelUpload(req, res, next) {
    const file = req.file;
    if (!file)
        ...
        for (const sheet of sheetNames) {
            ...
            for (let i = 0; i < Math.min(10, json.length); i++) {
                ...
                if (row && row.length > 0) {
                    ...
                    const hasAllHeaders = expectedHeaders.every((header) => row.includes(header));
                    if (hasAllHeaders) {
                        headerRow = row;
                        headerRowIndex = i;
            ...
            for (const [rowIndex, row] of dataRows.entries()) {
                ...
                const hasData = row && row.length > 0 && row.some((cell) => cell !== null && cell !== '' && cell !== undefined);
                if (!hasData) {
                ...
                const numericColumns = row.slice(4);
                ...
                if (sumIndex >= 0 && monthIndices.length > 0) {
                    ...
                    const reportedSumValue = row[sumIndex];
